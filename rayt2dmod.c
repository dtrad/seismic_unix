...
/* RAYT2D $Revision: 1.12 $; Date: 94/10/11 $	*/
...
"									",
" fzt=fz		first depth sample in traveltime table		",
" nzt=nz		number of depth samples in traveltime table	",
" dzt=dz		depth interval in traveltime table		",
" fxt=fx		first lateral sample in traveltime table	",
" nxt=nx		number of lateral samples in traveltime table	",
" dxt=dx		lateral interval in traveltime table		",
"									",
...
main(int argc, char **argv)
{
	...
	if(!getparint("nxt",&nxo)) nxo = nx;
	if(!getparint("nzt",&nzo)) nzo = nz;
	if(nxo<3 || nzo<3) err("nxt and nzt must be not less than 3!\n");
	if(!getparfloat("fxt",&fxo)) fxo = fx;
	if(!getparfloat("fzt",&fzo)) fzo = fz;
	if(!getparfloat("dxt",&dxo)) dxo = dx;
	if(!getparfloat("dzt",&dzo)) dzo = dz;
	exo = fxo+(nxo-1)*dxo;
	ezo = fzo+(nzo-1)*dzo;
	...
	if( !getparstring("tfile",&tfile)) {
		...
		if(restart[0] =='y') {
			...
			isize = (int) (ftell(tfp)/sizeof(float)/nsize);
			ixs0 = isize;
		}else {
			fclose(tfp);
			tfp = fopen(tfile,"w");
		} 
		fseek(tfp,(long) (isize*nsize*sizeof(float)),SEEK_SET);
	}
