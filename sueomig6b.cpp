...
void eomigls_op(float *m,float *mm,float *t, float *h, float *q, float *vel,
	     ...
	     float dperv, int norm, unsigned short nmute, unsigned short mute);
...
int main(int argc, char **argv)
{
  ...
  if (!getparushort("nmute",&nmute)) nmute=4;
  ...
  for (ix=0;ix<nx;ix+=ncsp){
	...
	eomigls_op(m,mm,t,he,q,velatcsp[icsp],csp[icsp],nt,nhcsp,nq,fmax,step,eps,eps1,eps2,itercg,iter_end,smooth,testadj,buffer,pervmin,dperv,norm,nmute,mute);

      if (output==6 || output ==7 ) 
	eomgetvel_op(m,mm,t,he,q,velatcsp[icsp],csp[icsp],nt,nhcsp,nq,fmax,step,eps,eps1,eps2,itercg,iter_end,smooth,testadj,buffer,pervmin,dperv,norm,itervel);    
