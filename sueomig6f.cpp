...
//int nq, method, rtmethod, freqflag;
//float depth;
...
int main(int argc, char **argv)
{
  ...
  for (ix=0;ix<nx;ix+=ncsp){
	...
	for (itr=0;itr<nhcsp;itr++){
      ...
      //testpointer(csp[icsp],nt,nhcsp);
      if (output==3 || output==4 || output==5)
	radonfreq(he,nhcsp,csp[icsp],t,nt,dt,velatcsp[icsp],eps1,eps2,eps,mm,q,nq,itercg,iter_end,step,lsmethod,factor,smute,nmofactor,mute,parmute);
