...
char *sdoc[] = {
  " 	   								",
  " SURADONCGFFT_TFD2 - Time domain Parabolic Radon Transform           ",
  "                     (works on susccessive CDPs in a line)           ",
  " 	   								",
  " suradoncgfft_tfd2 < stdin > stdout [optional parameters]   		",
  "                                                                     ",
  " IMPORTANT:****************************************                  ",
...
int main(int argc, char **argv)
{
  ...
  int plot; // =1 plots and saves gathers before and after mute
  // wavelet 
  int nw=0;
  float *wavelet=NULL;
  //char *name_file_wavelet=NULL;
  int typewav=1;
  float fpeak=20;
  ...
  if (!getparfloat("eps1", &inv.eps1))  inv.eps1 = 5e-1;
  if (!getparfloat("eps2", &inv.eps2))  inv.eps2 = 5e-1;
  if (!getparfloat("step", &inv.step))  inv.step = 0.98;
  if (!getparint("itercg", &inv.itercg))  inv.itercg = 15;
  ...
  if (!getparfloat("smute",&smute)) smute=20;
  ...
  if (!getparfloat("quantil",&quantil)) quantil=1;
  if (!getparint("typewav",&typewav)) typewav=4; // If typewav != 4 apply convolution
  if (!getparint("plot",&plot)) plot=1; // plot and save gathers before 
                                  // and after mute (in doublemute)
  ...
  // search for the first cdp  
    ...
    radoncgfft0_tfd_mute_conv(cmp,h,nhcmp[ix],t,nt,dt,mm,q,nq,velint,fmax,quantil,factor,
			 smute, nmofactor,rtmethod,depth,inv,0,parmute,0,mute,t0mute,
			 wavelet,nw,plot);
