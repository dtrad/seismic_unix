...
char *sdoc[] = {
  ...
  "                 (if resampling to a new offset is desired           ",
  NULL};
/* Credits:
 *	Daniel Trad.
...
int main(int argc, char **argv)
{
  ...
  if (offsetfile){
    nhnew=read_ascii_file(offsetfile,h);  
    fprintf(stderr,"Using %s offsetfile with %d offsets\n",offsetfile,nhnew);
  }
  ///////////////////////////////////////////////////////////////////////
  ...
  // search for the first cdp  
    ...
    if (offsetfile) radoninv0(hnew,nhnew,cmp,t,nt,dt,velint,mm,q,nq,smute,nmofactor,depth,fmax,rtmethod);
    // Output trace
    //erewind(tracefp);
    erewind(headerfp);  
