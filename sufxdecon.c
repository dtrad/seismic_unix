t/* Copyright (c) Colorado School of Mines, 2000.*/
/* All rights reserved.                       */

/* SUFXDECON: $Revision: 1.4 $ ; $Date: 2000/09/22 19:35:52 $	*/	
...
main(int argc, char **argv)
{
	...
	nstaper=(int)(taper/dt);
	nstaper = (nstaper%2 ? nstaper+1 : nstaper); 

	ntw = (int)((nsp-1)*dt/twlen);

	if (ntw==1) taper=0.0;

	nspws = (int)(twlen/dt) + 1;
	...
	/* zero output file */
	memset((void *) tdataout[0], (int) '\0', ntrc*nsp*FSIZE);
	...
	for (iw=0;iw<ntw;iw++) {
		...
		/* zero fdata */
		memset((void *) fdata[0], (int) '0', nf*ntrc*sizeof(complex));
		...
		for (jw=0;jw<ntrc;jw++) {
			...
			memset((void *) (tidataw + nspw), (int) '0', (nfft-nspw)*FSIZE);
		...
		for (jx=0;jx<nwx;jx++){
			...
			for (jw=0;jw<ntrw+2*ntrf;jw++)
			        memset((void *) fdataw[jw], (int) '0', nf*sizeof(complex));
			...
			for (ifq=0;ifq<nf;ifq++) {
			        ...
			        if ((float)ifq*d1>=fmin && (float)ifq*d1<=fmax) {
				       ...
				       memset((void *) sfreq, (int) '0', (ntrwu+2*ntrf)*sizeof(complex));
				       memset((void *) sfreqcj, (int) '0', (ntrwu+2*ntrf)*sizeof(complex));
				       ...
				       memset((void *) autocorr, (int) '0', (ntrf+1)*sizeof(complex));

				       /* complex autocorrelation */
				       cxcor(ntrwu,0,sfreq,ntrwu,0,sfreq,ntrf+1,0,autocorr);

				       /* zeroing files */
				       memset((void *) rautoc, (int) '0', (ntrf+1)*FSIZE);
				       memset((void *) iautoc, (int) '0', (ntrf+1)*FSIZE);
				       ...
				       /* zeroing files */
				       memset((void *) gvector, (int) '0', 2*ntrf*FSIZE);
				       memset((void *) fvector, (int) '0', (2*ntrf+1)*sizeof(complex));
				       for (ir=0;ir<2*ntrf;ir++) 
					       memset((void *) rmatrix[ir], (int) '0', 2*ntrf*FSIZE);
				       ...
				       memset((void *) sfreqout, (int) '0', ntrwu*sizeof(complex));
			...
			for (jw=0;jw<ntrwu;jw++) {
				...
				memset((void *) (freqv+nf), (int) '0', (nfft-nf)*sizeof(complex));
				memset((void *) todataw, (int) '0', nfft*FSIZE);

				/* FFT back from f to t and scaling */
				pfacr(-1, nfft, freqv, todataw);
				for (it=0;it<nsp;it++) todataw[it]/=nfft; 
				for (it=0;it<nsp;it++)
