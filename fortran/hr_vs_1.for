...
c      *************************************************
        ...
        call V_STACK_O(d,nh,pos,nt,dt,m,nq,qmin,qmax,eps1,iter_end)
...
c-----------------------------------------------------------------------------
      ...
      subroutine  V_STACK_O(d,nh,pos,nt,dt,m,nq,qmin,qmax,eps1,iter_end)
c
c
c       Mauricio D. Sacchi, Geophysics and Astronomy, UBC
c       E-mail: sacchi@geop.ubc.ca
...
c------ solve dc=L.mc using Cauchy-Gauss model
        ...
        call CAUCHY_GAUSS_offset(l,lh,dc,mc,nh,nq,eps1,iter_end)
...
c-----------------------------------------------------------------------------
      ...
      subroutine CAUCHY_GAUSS_offset(l,lh,d,model,nd,np,eps1,iter_end)
c
c       Mauricio D. Sacchi, Geophysics and Astronomy, UBC
c       E-mail: sacchi@geop.ubc.ca
c       Tel (o): (604)-822-2267
...
c	Remember that Qp is diagonal and inv(Qp)=(1/sigma^2)(1+(|v|/sigma)^2
      ...
      complex *16  a(8256),model(nnx),d(nnx),L(nnx,nnx),lh(nnx,nnx) 
...
200   continue
      ...
      call  CHOLESKY (nd,1.d-15,A,aux,ISTAT)  ! Solution to (LH.inv(Qp).L).x=d
...
c-----------------------------------------------------------------------------
      ...
      subroutine matrix_2(L,lh,pmin,pmax,pos,np,nh,w)
