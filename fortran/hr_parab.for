...
10      read(20,*) (d(it,ih),ih=1,nh)
        ...
        enddo
        do ih=ie+1,nh
        d2(it,ih-ie-1+is)=d(it,ih)
        enddo
        enddo

        do ih=1,is-1
        pos(ih)=near_offset+(ih-1)*dh
        enddo

        do ih=ie+1,nh
        pos(ih-ie-1+is)=near_offset+(ih-1)*dh
        enddo

        nh_gap=nh-(ie-is+1)+1
 
C
c       1 2 3 4 . . . is. . . ie ie+1 . . . . nh
c                      |       |
c                      0 0 0 0 0
c
...
c       write(42,*) nh,nt
c       write(42,*) dh,dt
c       write(42,*) near_offset,t0
c       do  ih=1,is-1
c       write(42,31) (d2(it,ih),it=1,nt)
c       enddo
c       do  ih=is,ie
c       write(42,31) (0.,it=1,nt)
c       enddo
c       do  ih=is+1,nh_gap
c       write(42,31) (d2(it,ih),it=1,nt)
c       enddo
        ...
        index=1
        call nmoc(pos,d2,nt,nh_gap,dt,t0,dh,
     #          near_offset,tv,v,nv,d_nmoc,index)
	 ...
	 close(43)
c       do  ih=is,ie
c       write(43,31) (0.,it=1,nt)
c       enddo
c       do  ih=is+1,nh_gap
c       write(43,31) (d_nmoc(it,ih),it=1,nt)
c       enddo
...
c      *************************************************
        ...
        call P_STACK(method,pos,d_nmoc,nh_gap,
     #       nt,near_offset,dh,dt,
     #       m,nq,qmin,qmax,eps1,eps2,iter_end)

c      *************************************************

        open(30,file=fileout_1,form='binary',status='unknown')
c       open(40,file=fileout_2,status='unknown')
        write(30) nq,nt
        write(30) dq,dt
        write(30) qmin,t0
        do 12 iq=1,nq
...
1       do i=1,nh
        pos(i)=near_offset+(i-1)*dh
        enddo

c       call sum_slope(d_rec,nh,nt,m,nq,
c    #     qmin,qmax,dt,dh,near_offset)

        call  H_STACK(pos,d_rec,nh,nt,near_offset,dh,dt,
     #               m,nq,qmin,qmax)

c       write(45,*) nh,nt
c       write(45,*) dh,dt
c       write(45,*) near_offset,0.
c       do 14 ih=1,nh
c4      write(45,31) (d_rec(it,ih),it=1,nt)


        index=-1
        call nmoc(pos,d2,nt,nh,dt,t0,dh,
     #          near_offset,tv,v,nv,d_rec,index)
...
c------ write the reconstructed CMP gather. d_rec(nt,nh)
        ...
        do 18 ih=1,nh
18      write(40) (d2(it,ih),it=1,nt)
         close(40)

c       write(41,*) nh,nt
c       write(41,*) dh,dt
c       write(41,*) near_offset,0.
c       do 15 ih=1,nh
c5      write(41,31) (d(it,ih)-d2(it,ih),it=1,nt)


31      format(10f15.5)
81      format(i6,2f15.8)

c       write(44,*) (ie-is+1)*2+8,nt
c       write(44,*) dh,dt
c       write(44,*) 0.,0.
c       do ih=1,2
c       write(44,31) (0.0d0,it=1,nt)
c       enddo
c       do ih=is,ie
c       write(44,31) (d(it,ih),it=1,nt)
c       enddo
c       do ih=1,4
c       write(44,31) (0.0d0,it=1,nt)
c       enddo
c       do ih=is,ie
c       write(44,31) (d2(it,ih),it=1,nt)
c       enddo
c       do ih=1,2
c       write(44,31) (0.0d0,it=1,nt)
c       enddo
...
c-----------------------------------------------------------------------------
        ...
        subroutine  P_STACK(method,pos,d,nh,nt,near_offset,dh,dt,
     #              m,nq,qmin,qmax,eps1,eps2,iter_end)
c
c
c       Mauricio D. Sacchi, Geophysics and Astronomy, UBC
...
c------ compute the linear operator for non-equally spaced data
        ...
        call matrix_nu(pos,L,qmin,qmax,nq,nh,dh,w,near_offset)

c------ solve dc=L.mc using Cauchy-Gauss model

        if(method.eq.2) then 
        call GAUSS_GAUSS(l,dc,mc,nh,nq,eps1)
        call CAUCHY_GAUSS_2(l,dc,mc,nh,nq,eps1,iter_end,eps2)
        endif
...
c     retrieve teh solution. 
      ...
      real    * 8  power(nnx),p(nnx),pm,smin
...
c      do 10 j=1,nd
c      model(i)=model(i)+dconjg(l(j,i))*d(j)
c10    continue
      ...
      do i=1,np
       power(i)=dreal(model(i)*dconjg(model(i)))
        ...
        enddo
         ...
         write(*,*)' max: ',s, ' min: ',smin, ' eps2 : ', eps2
            
c        if(eps3.eq.0.d0) eps2=pm
c        if(eps3.ne.0.d0) eps2=eps3*pm
...
c-----------------------------------------------------------------
        ...
        subroutine matrix_nu(pos,L,pmin,pmax,np,nh,dh,w,near_offset)
...
c       p= 1/(velocity)^2
        ...
        complex * 16 l(nnx,nnx)
        real * 8     dp,arg,dh,w,pmin,pmax,p,h,near_offset
        real * 8     pos(nnx)
        ...
        h=pos(i)
        arg=w*(h**2)*p
100     l(i,j)=cdexp(dcmplx(0.d0,-arg))
...
c---- to is the diagonal term of the Toeplitz form (the zero lag).
      ...
      sum=dcmplx(0.d0,0.d0)
      do 300 k=1,nd
300   sum=sum+dconjg(l(k,i))*l(k,1)
      if(i.eq.1.) then
      t0=sum*(1.d0+eps1/100.)   ! damping
      endif
      if(i.ne.1) t(i-1)=sum
...
c---- solve the Toepliz form using Levinson
...
c---------------------------------------------
     ...
     #          near_offset,tv,v,nv,data_c,index)
...
c------ index .eq.-1  ==> data_c--->data
        ...
        real * 8 t0,t,dt,dh,tau,h,near_offset,vi(nnt)
        real * 8 pos(nnx)
...
c--------------------------------------------------------
        ...
        subroutine  H_STACK(pos,d,nh,nt,near_offset,dh,dt,
     #              m,nq,qmin,qmax)
c
c
c       Mauricio D. Sacchi, Geophysics and Astronomy, UBC
