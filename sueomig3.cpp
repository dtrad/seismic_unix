...
int main(int argc, char **argv)
{
  ...
  if ((csp=ealloc2float(nhcsp,nt))==NULL)
  ...
  for (ix=0;ix<nx;ix++){
    ...
    for (ih=0;ih<nhcsp;ih++) for (it=0;it<nt;it++) csp[it][ih]=0; // CSP trace
	...
	puttr(&tr);      
      ...
      if (testhe) 
	equiv_offset_test(tr,csp,t,x[ix],velint,dt,he,dhe,nt,nhcsp,beta,cdpspace);
      else if (testhe==2)
	equiv_offset_1side(tr,csp,t,x[ix],velint,dt,he,dhe,nt,nhcsp,tc,hec,precise,beta,cdpspace);   
      else 
	equiv_offset(tr,csp,t,x[ix],velint,dt,he,dhe,nt,nhcsp,tc,hec,precise,beta,cdpspace);    
    }
    erewind(tracefp);
    erewind(headerfp);
    ...
    if (output==3 || output==4 || output==5) 
      eomigls(m,mm,t,he,q,velint,csp,nt,nhcsp,nq,fmax,step,eps,eps1,eps2,itercg,iter_end,smooth,testadj,buffer);

    if (output==6) 
      eomgetvel(m,mm,t,he,q,velint,csp,nt,nhcsp,nq,fmax,step,eps,eps1,eps2,itercg,iter_end,smooth,testadj,buffer);    

    if (output==1 || output==6) eomig1(csp,he,m,t,velint);
    ...
    if ((output==1)||(output==3)||(output==6)){
      for (it=0;it<nt;it++)
	m[it]/=(fold[ix]*velint[0]*sqrt(pi*MAX(t[it],1e-2)));
