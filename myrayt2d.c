...
void raytime2d(Raytr raytr,Geo2d geo2dv,float *vt,Geo2d geo2dt,float *t,
	       int npv,float *vo,float *pv,float *tv,float *cs, float* output);
...
main(int argc, char **argv)
{
	...
	float * output = 0; /* array to save raw rays for testing */
	...
	output = alloc1float(nxo*nzo);
	for (int i=0;i<nxo*nzo;i++) output[i] = v[i];
	//memset((void*) output,(int) '\0',nxo*nzo*FSIZE);
	/* loop over sources */
	for(ixs=ixs0,xs=fxs+ixs0*dxs;ixs<nxs;ixs++,xs+=dxs){
		...
		raytime2d(raytr,geo2dv,vt,geo2dt,t,npv,vo,pvt,tv,cs,output);
	...
	fclose(vfp);
	fclose(jpfp);
	/* save output with raw shots */
	FILE* myfp = fopen("output","w");
	if (myfp){
	  fwrite(output,sizeof(float),nxo*nzo,myfp);
	  fclose(myfp);
	}
	else fprintf(stderr,"error opening output\n");
	free1float(output);
	free1float(v);
	free1float(t);
	if(npv){
...
void saveRay(Ray* ray, Geo2d geo2dt, int nrs, float* output){
  int ixo, izo;
  int nxo = geo2dt.nx;
  int nzo = geo2dt.nz;
  float dxo = geo2dt.dx;
  ...
  ixo = ray[0].x/dxo;
  izo = ray[0].z/dzo;
  fprintf(stderr,"ray[0].x=%f, ray[0].z=%f ixo=%d izo=%d \n",ray[0].x,ray[0].z,ixo,izo);
  for (int i=0;i<nrs;i++){
    
    ixo = ray[i].x/dxo;
    izo = ray[i].z/dzo;
    if ( (ixo < nxo) && (ixo>0) && (izo < nzo) && (izo>0) ) output[ixo*nzo+izo] = i*0.004;//ray[i].t;
...
/*		trace rays	*/
		...
		saveRay(ray,geo2dt,nrs,output); /* save raw rays to a file for testing */
...
/*	 if the point is out of output range	*/
		  ...
		  zc = zo;
		  q2d = ray[it].q2; 
		  if(q2d == 0.0){  r2 = 0.0;continue;} /*  if caustics	*/
...
/*		  determine radius for extrapolation	*/
		  tc = it*dt;
		  terr = tc*fac;
		  norm2 = sqrt(d2t[0]*d2t[0]+d2t[2]*d2t[2]+2.0*d2t[1]*d2t[1]);
