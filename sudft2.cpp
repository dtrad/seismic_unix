...
int main(int argc, char **argv)
{
  ...
  for (ih=0;ih<nh;ih++) for(it=0;it<nt;it++) Wd[it][ih]=1;
  ///////////////////////////////////////////////////////////////////////
  ...
  // compute k axis
  if (!dx) dx = h[1]-h[0];
  if (!dx) err("can't define h from data\n");
  dk = 1./(nk*dx)*2*PI;
  kmin = (-nk/2+1)*dk;
  ...
  dft2_interface(data, model,h,nh,t,nt,k,nk,velint,smute,nmofactor,fmax,data2,h2,nh2,
		 Wd,inv,method,df);
  TRACE;
  // Output trace
  ...
  */
  ...
  for (ik=0;ik<nk;ik++){
    ...
    for (it=0;it<nt/2;it++) tr.data[it]=abs(model[it][ik]);
    tr.d1=df;       
    tr.ntr=nk;
    tr.ns=nt/2;
    tr.tracl=ik+1;
