...
int main(int argc, char **argv)
{
  ...
  model=alloc2complex(nw,nk);
  m=ealloc1float(nt);
  t=ealloc1float(nt);
  w=ealloc1float(nw);
  k=ealloc1float(nk);
  h=ealloc1float(nh);
  Wm=ealloc2float(nt,nk);
  ...
  fft2_0(data,model,h,nh,t,nt,k,nk,w,nw,velint,eps1,smute,nmofactor,fmax,data2,h2,nh2);
  dk=k[1]-k[0];
  dw=w[1]-w[0];
  fw=w[0];
  fk=k[0];
  ...
  */
  ...
  for (ik=0;ik<nk;ik++){
    ...
    for (iw=0;iw<nw;iw++) tr.data[iw]=abs(model[ik][iw]);
    /* set header values */
    tr.tracl = ik + 1;
    tr.ns = nw;
    tr.ntr= nk;
