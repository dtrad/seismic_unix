...
/* All rights reserved.                       */
/* supocs2c0  :  $Date: July    2012- Last version July    2012  */
...
bool process(float** datain, float** datain2, float** dataout, float** dataout2, int nt, int  nh, float dt);
/*********************** self documentation **********************/
const char *sdoc[] = {
  " 	   								",
  " SUPOCS2C  vector reconstruction of 2C data                          ",
  "                                                                     ",
 NULL};
/* Credits:
 *	Aaron Stanton.
...
int main(int argc, char **argv)
{
  ...
  for (ih=0;ih<nh;ih++){ 
    fgettr(fp2,&tr);
    memcpy((void *) tr.data,(const void *) dataout2[ih],nt*sizeof(float));
    tr.offset=(int) h[ih];
    tr.ntr=nh;
    if (Wd[ih][0]==0) tr.trid=2; // dead trace
    fputtr(fpout2,&tr);
