...
bool process(float** datain, float** dataout, int nt, int  nx, float dt){
  ...
  int extIter = 5; // external iterations for thresholding
  ...
  if (plotInput){
    ...
    system("suximage < input.su perc=98 legend=1 title=\"input\" &");
  }
  ...
  if (plotInput2){
    ...
    system("suximage < input2.su title=\"after zeros\" legend=1 perc=98 &");
  }
  ...
  if (plotFKIn){
    xplotgather(cpfft,nk,nw,df,dx,"datafxin.su",1,"perc=99 &"); // plot [x,w]
    xw2kw(cpfft,nxfft,nk,nw,freqslice,-1);                      // x to k
    xplotgather(cpfft,nk,nw,df,dk,"datafkin.su",1,"perc=99 &"); // plot [k,w]
  ...
  for (iw=0;iw<nw;iw++){
    ...
    for (int iter2=1;iter2< extIter;iter2++){  // thresholding loop
  ...
  if (plotFKOut){   // plot final spectra
    xw2kw(cpfft,nxfft,nk,nw,freqslice,-1); // x to k
    xplotgather(cpfft,nk,nw,df,dk,"datafkout.su",1,"perc=99 &");
    xw2kw(cpfft,nxfft,nk,nw,freqslice,1);                       // k to x
  }
  ...
  system("suximage < output.su title=\"after pocs\" legend=1 perc=98 &");
