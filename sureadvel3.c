...
main(int argc, char **argv)
{
	...
	*/
	...
	if ((vz=ealloc2float(nz,nx))==NULL)
	  err("Cannot allocate space for vz\n");
	if ((z=ealloc1float(nz))==NULL)
	  err("cannot allocate space for z\n");
	jump=(int) (floor(ntr/nx+0.5));

	for (z[0]=0,iz=1;iz<nz;iz++) z[iz]=z[iz-1]+dz;
	fprintf(stderr,"dz=%f;z[nz-1]=%f,jump=%d\n",dz,z[nz-1],jump);
	...
	efwrite(vz[0],FSIZE,nz*nx,pvfile);
	efclose(pvfile);
	...
	for (itr=0;itr<ntr;itr++){
	  ...
	  if ((onlycdp)&&(tr.cdp==onecdp))  printf("%s=%g", string1, x1);
	  else if (onlycdp==0) printf("%s=%g", string1, x1);
	  for (i1 = 2; i1 < n1; i1++) {
	    efread(&x1, FSIZE, 1, datafp1);
	    if ((onlycdp)&&(tr.cdp==onecdp)) printf(",%g", x1);
	    else if (onlycdp==0) printf(",%g", x1);    
	  }
	  if ((onlycdp)&&(tr.cdp==onecdp)) putchar('\n');	
	  else if (onlycdp==0)  putchar('\n');	
	  /* Rewind and get the x2's */
	  rewind(datafp2);
	  efread(&x2, FSIZE, 1, datafp2);

	  if ((onlycdp)&&(tr.cdp==onecdp)) printf("%s=%g", string2, x2);
	  else if (onlycdp==0) printf("%s,%g", string2, x2);
