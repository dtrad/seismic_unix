...
char *sdoc[] = {
  " 	   								",
  " SURADONLOGBAR - Frequency domain Parabolic Radon Transform          ",
  " 	   								",
  " suradonsolver < stdin > stdout [optional parameters]      		",
  "                                                                     ",
...
{
  segy tr;
  logb_param par; /* structure with logbarrier parameters */
  ...
  if (!getparfloat("delta", &par.delta))  par.delta = 1.0;
  if (!getparfloat("gamma", &par.gamma))  par.gamma = 1e-4;
  if (!getparfloat("PI_t", &par.PI_t))   par.PI_t= 1e-2;
  if (!getparfloat("DI_t", &par.DI_t))   par.DI_t= 1e-2;
  if (!getparfloat("GI_t", &par.GI_t))   par.GI_t= 1e-2;
  if (!getparint("itercg", &par.itercg))  par.itercg = 15;
  if (!getparint("iter_end", &par.iter_end))  par.iter_end = 2;
  ...
  if (!getparstring("solver",&solver)) solver="logbar";

  /* Format the string solver to have the same length */ 
  if (STREQ(solver,"adj")) solver="adj___";
  ...
  radonlogbar0(data,h,nh,t,nt,dt,model,q,nq,velint,factor,smute,nmofactor,rtmethod,depth,
	       fmax,solver,par);  
