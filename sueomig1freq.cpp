...
int main(int argc, char **argv)
{
  ...
  if (tests==0){
    if (1){
      stack_rt_operator(oper,maxfreq,df,h,nh,q,nq,t,nt,eps,parmute2);
      save2dfile(oper,nh,maxfreq,"oper");
    }
    if (0) read2dfile(oper,nh,maxfreq,"oper");
  }
  else if (tests==1){
    ...
    if (compute) 
      stack_rt_LS_operators(LSoper,Loper,maxfreq,df,h,nh,q,nq,t,nt,eps,parmute2,mutevector,
			    stackvector);
    ...
    /* and read them */
    if (!compute){
      mute_vector(mutevector,q,nq,parmute2);
