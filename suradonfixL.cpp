...
char *sdoc[] = {
  " 	   								",
  " SURADONFIXL  -  Frequency domain Parabolic Radon Transform          ",
  " 	   	    with fixed operator L (dq variable with freq)	",
  "                                                                     ",
  " suradonsolver < stdin > stdout [optio al parameters]          	",
  "                                                                     ",
...
int main(int argc, char **argv)
{
  ...
  if (!getparstring("solver",&solver)) solver="cgfft_";
  ...
  // This is a default for q, in case it is not defined later, but in general 
  //is changed  later
  dq = (qmax-qmin)/(nq-1);
  ...
  radonsolver0(data,h,nh,t,nt,dt,model,q,nq,velint,itercg,iter_end,step,eps2,eps1, 
	       quantil,norm,factor,smute,nmofactor,rtmethod,depth,fmax,solver);  
