...
void rad_ellip_sp(float *t,float *p,float *v, float **vgrid,int nt, int np, int nv, unsigned short **index, int nsparse, unsigned short *nullspace);
...
int main(int argc, char **argv)
{
  ...
  size_t sizeus=sizeof(unsigned short);
  float *J;
  /// Velocity Trend
  float *tvel;
  float *vel;
  ...
  if ((daux=ealloc1float(np*nt))==NULL)
    err("Cannot allocate daux\n");

  if ((nullspace=(unsigned short*) alloc1(np*nt,sizeus))==NULL)
  ...
  if ((J=ealloc1float(iter_end+1))==NULL)
    fprintf(stderr,"Cannot alloacte J\n");
  ...
  if (method==0){ 
    ...
    if ((index=(unsigned short **) alloc2(nsparse,2,sizeus))==NULL)
      err("Cannot allocate the monster index\n");
    rad_ellip_sp(t,p,v,vgrid,nt,np,nv,index,nsparse,nullspace);
    ...
    for (j=1;j<=iter_end;j++){
      ...
      J[j]=wpcgnr(radonellip0,nt,np,nv,nsparse,m,d,Wd,Wm,index,eps,step,itercg,
	     restart);
    }

    
    free2((void **) index);
    radonellip1(m,t,p,v,daux,vgrid,0,nt,np,nv);
    for (i=0;i<np*nt;i++) if (nullspace[i]) d[i]=daux[i];
  }
  else if (method==1){
    ...
    for (j=1;j<=iter_end;j++){
 	modelweight(m,nx,norm,eps1,Wm);
	J[j]=wpcgnr(radonellip1,nt,np,nv,t,p,v,m,d,Wd,Wm,vgrid,eps,step,itercg,
	        restart);
  ...
  if (1) save_vector(&J[1],iter_end,"costfunc");
...
void rad_ellip_sp(float *t,float *p,float *v, float **vgrid,int nt, int np, int nv, unsigned short **index, int nsparse, unsigned short *nullspace)
{
  ...
  for (it=0;it<(np*nt);it++) nullspace[it]=0;
