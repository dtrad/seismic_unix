...
char *sdoc[] = {
  " 	   								",
  " SURADONCGFFT - Time  domain Parabolic Radon Transform               ",
  " 	   	   (works on one CDP) 					",
  " suradoncgfft_tfd   < stdin > stdout [optional parameters]  		",
  "                                                                     ",
...
int main(int argc, char **argv)
{
  ...
  // The following parameters are defined in struct inv_par
  //  float step;
  //  float eps1;
  //  float eps2;
  ...
  if (fmax==0) fmax=1./(2*dt); // To set fmax to Nyquist choose fmax=0
  ...
  if (!getparint("stopc",&stopc)) stopc=1;
  if (!getparfloat("depth",&depth)) depth=2000;
  ////////////////////////////////////////////////////////////////////////
  // The parameter stopc is stored in inv.taperflag 
  inv.taperflag=stopc;
  ...
  radoncgfft0_tfd(data,h,nh,t,nt,dt,model,q,nq,velint,fmax,quantil,factor,smute,
		   nmofactor,rtmethod,depth,inv);  
  ...
  if (0) plotgather(model[0],nt,nq,"model2");
