...
int main(int argc, char **argv)
{
  ...
  *******************************************************************/
  ...
  if (calcMute){
    ...
    float sum=0;
    for (ih=par.ihmin;ih<nh;ih++) for (it=0;it<nu;it++) sum+=fabs(datains[ih][it]);
    threshold=mutePick*sum/(nu*(nh-par.ihmin));
    
    for (ih=0;ih<nh;ih++){
      for (it=0;it<nu;it++){
	if (fabs(datains[ih][it]) > threshold){
	  mutetime[ih]=it*du;
	  break;
	}
      }
    ...
    xplotgather(datains,nh,nu,du,"datainfirstBreak",0,buf);
  ...
  if (1)  xplotgather(dataouts,nh,nu,du,"dataouts.su_MODEL",0,"perc=97 xbox=600 legend=1&");
  /**************************************************************/
  ...
  if (1)  xplotgather(dataout,nh,nt,dt,"dataout.su_FIRST_pass",0,"perc=97 xbox=400 legend=1 ");  
