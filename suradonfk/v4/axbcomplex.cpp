...

{
	...
	complex dum;	/* pivot scale factor */
	complex sum;	/* auxiliary variable for summ  */
	...
	*d = 1.0;
	...
	for (j=0; j<nrows; j++) {
		...
		for (i=j; i<nrows; i++) {
			...
			dum.r=vv[i]*abs(sum);dum.i=0;
			if ((dum.r=vv[i]*abs(sum)) >= big) {
 				big = dum.r;
				imax = i;
			}
		}
...

{
	...
	complex sum;		/* auxiliary variable for partial sums */
...
************************************************************************/
void inverse_matrix (int nrows, complex **matrix)
/************************************************************************
Input:
...
************************************************************************/
{
	...
	complex *column;		/* unit vector for backward substitution*/
	complex **inverse;	/* array to hold the inverse matrix */

	/* allocate working space */
	idx = alloc1int(nrows);
	column = alloc1complex(nrows);	
	inverse = alloc2complex(nrows,nrows);
...

{
	...
	complex **scratch1;	/* array to hold input matrix1 */
	complex *scratch2;	/* vector to hold column of input matrix2 */

	/* allocate working space */
	idx = alloc1int(nrows1);
	scratch1 = alloc2complex(nrows1,nrows1);
	scratch2 = alloc1complex(nrows2);
	...
	/* free allocated space */
	free2complex(scratch1);
	free1complex(scratch2);
}
