...
" nrad=20               number of Radon traces                          ",
" dv=100                increment in velocities                         ",
" vmin2=1000            minimum velocity                                ",
" plot=0                plot=1 plot of independent gathers              ",
...
main(int argc, char **argv)
{
  ...
  if (!getparfloat("vmin2",&vmin2)) vmin2=1000;
  ...
  free1float(v);
  // vstolt-=(int) (nrad/2*dv);;
  if (vmin2) vstolt=vmin2;
  ...
  for (irad=0;irad<nrad;irad++){
	...
	if (nu!=nt && tu!=NULL) {
	  ...
	  for (ix=0; ix<nx; ++ix) {
	    ints8r(nt,dt,0.0,px[ix],0.0,0.0,nu,tu,temp);
	    for (iu=0; iu<nu; ++iu)
	      px[ix][iu] = temp[iu];
	  }
	  free1float(temp);
	...
	fprintf(stderr,"vstolt=%f\n",vstolt);
	for (ik=1,k=dk; ik<nk; ++ik,k+=dk){
	  fprintf(stderr,"ik=%d.....\n",ik);
          //TRACE;
	  if (adj) stolt1kadj(k,vstolt,smig,fmax,nu,du,pk[ik],pk[ik]);
          else stolt1k(k,vstolt,smig,fmax,nu,du,pk[ik],pk[ik]);
	}  
	...
	if (0){
	  save_gather(q,nx,nt,0.004,"migrated.su");
	...
	for (it=0; it<nt; ++it)
	  tro.data[it] = 0;
	for (it=0; it<nt; ++it)
	  tro.data[it] += q[NINT(ntrace/2)][it];
	
	/* write output trace */
	puttr(&tro);	
	if (0)// Cancel the following loop 
