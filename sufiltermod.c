...
char *sdoc[] = {
"									",
" SUFILTERMOD - applies a zero-phase, sine-squared tapered filter	",
"									",
" sufiltermod <stdin >stdout [optional parameters]         		",
"									",
" Required parameters:                                         		",
...
"                                                                       ",
" added               if ((tr.offset==-1000)||(offset==-950))           ",   
" so that only these two traces are filtered                            ", 
NULL};
/* Credits:
...
main(int argc, char **argv)
{
        ...
        float *filter2;          /* filter array auxiliar               */
	...
	if (1){
	  ...
	  for (i=0;i<nf;i++) filter2[i]=1./nfft;
        ...
        do {
                ...
                pfarc(1, nfft, rt, ct);
		fprintf(stderr,"tr.offset=%d\n",tr.offset);
                if ((tr.offset==-1000)||(tr.offset==-950)||(tr.offset==-900)) 
		  for (i = 0; i < nf; ++i)  ct[i] = crmul(ct[i], filter[i]);
		else
		  for (i = 0; i < nf; ++i)  ct[i] = crmul(ct[i], filter2[i]);
                pfacr(-1, nfft, ct, rt);
