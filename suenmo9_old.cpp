...
int main(int argc, char **argv)
{
  ...
  if (0) critical_angle(t,nt,tvel,nvel,critang,critangle);
  if (1) for (it=0;it<nt;it++) critangle[it]=0.90;
		   ...
		   inv_par inv, int itercgfin)
{
  ...
  vgridw=ealloc2float(ntwin,nv);
  mw=ealloc2float(ntwin,nv);
  dw=ealloc2float(ntwin,np);
  Wdw=ealloc2float(ntwin,np);

  for (iwin=0;iwin<nwin;iwin++){
    
    it0=iwin*ntwin+it0_total; 
    for (ip=0;ip<np;ip++) for (it=0;it< ntwin; it++) dw[ip][it]=d[ip][it+it0];
    for (ip=0;ip<np;ip++) for (it=0;it< ntwin; it++) Wdw[ip][it]=Wd[ip][it+it0];

    if (0){
      save_gather(dw,np,ntwin,dt,"dw");
      system("suxwigb < dw  title=\"plot dw\" ");
    }

    for (iv=0;iv<nv;iv++) for (it=0;it< ntwin; it++) vgridw[iv][it]=vgrid[iv][it+it0];  

    //memcpy((void *) dw[ih],(const void *) &d[ih][it],ntwin*sizeof(float));
    TRACE;
    enmo_win(dw,mw,&t[it0],p,v,ntwin,np,nv,vgridw,critangle,Wdw,LI,inv,itercgfin);    
    TRACE;
    if (0){
      save_gather(mw,nv,ntwin,dt,"mw");
      system("suxwigb < mw  title=\"plot mw\" ");
    }

    for (iv=0;iv<nv;iv++) for (it=0;it< ntwin; it++) m[iv][it+it0]=mw[iv][it];
    for (ip=0;ip<np;ip++) for (it=0;it< ntwin; it++) d[ip][it+it0]=dw[ip][it];
  }
