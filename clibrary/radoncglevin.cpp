...
void radoncg_levinson(float *pos, int nh, float **data, float *t,float nt,float **model, float *q, float nq, float dq,float eps1, float eps2, float eps, float fmax, float *Wd, int itercg, int iter_end, int norm, float step, int testadj)
{
  ...
  for (freq=1;freq<maxfreq;freq++){
    ...
    while ((iter<=iter_end)&&(flag==0)){
      if (norm==10) eps2=modgrad(u,nq,norm,power,Qp);
