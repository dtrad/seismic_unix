...
void radoncgfft0_tfd_mute_conv(float **data, float *h, int nh,float *t, int nt, float dt, float **model, float *q, int nq, float *vel, float fmax,  float quantil, float factor, float smute, float nmofactor, int rtmethod, float depth, inv_par inv, float parmute1, float parmute2, int mute1, int mute2, float t0, float *wavelet, int nw, int plot)
{
  int i, it, ih;
  float  qmax, qmaxt;
  float  dx_av, dx_max;
  ...
  radoncgfft_tfd_conv(h,nh,data,t,nt,dt,model,q,nq,dq,fmax,Wd,testadj,quantil,rtmethod,depth,inv,wavelet,nw);

  //hrrti(data,h,dt,model,q,fmax,nt,nh,nq,rtmethod,depth);
  /* Any automatic filtering process can be done here on model[nq][nt] */
  if ((mute1)||(mute2))
       doublemute(data,model,nq,nh,nt,q,h,t,parmute1,parmute2,mute1,mute2,fmax,rtmethod,depth, t0, wavelet, nw, vel, nmofactor, smute, plot);
		    ...
		    float *wavelet, int nw)
{
  ...
  radon(model2[0],data[0],L,nt,nh,nq,dt,m2,d2,nfft,fmax,1);
  conv_corr(model2[0],model[0],nt,nq,wavelet,nw,TRUE);
  if (0) plotgather(model,nq,nt,dt,"suxwigb perc=99");
  fprintf(stderr,"quantil1=%f, quantil2=%f\n",quantil1,quantil2);

  for (iter=1;iter<=inv.iter_end;iter++){
    if (0)  if (iter==inv.iter_end) inv.taperflag=2; // change stop criteria at the last iteration
    if (iter==2) deviations(model[0],nq*nt,data[0],nh*nt,inv.norm,quantil1,quantil2,
			 &sigmam,&sigmad);
    fprintf(stderr,"sigmam=%f, sigmad=%f\n",sigmam,sigmad);
    weights_cgfft(model[0],nt*nq,inv.norm,sigmam,Wm[0],iter);    
    J[iter-1]=cgfft(data[0],L,model[0],d2,RC,m2,nt,nh,nq,nf2,dt,nfft,fmax,Wm[0],Wd[0],inv,wavelet,nw);
  }
...
{
  float normb, beta,betanum, alpha, alphanum, alphaden;
  int k,i;
  ...
  radonf(g,r,L,nt,nh,nq,dt,m2,d2,nfft,fmax,TRUE); // TRUE means adjoint
  conv_corr(g,stemp,nt,nq,wavelet,nw,TRUE);       // TRUE means correlation
  for(i=0;i<nx;i++) g[i]=stemp[i]*Wm[i];          // stemp is a temporal array
  ...
  while(k<inv.itercg){
    ...
    conv_corr(stemp,stemp3,nt,nq,wavelet,nw,FALSE); // convolution   
    Atimesx_by_fft_loop(stemp2,RC,stemp3,m2,m2aux,nt,nq,dt,nfft,fmax,nf2);
    conv_corr(stemp2,stemp3,nt,nq,wavelet,nw,TRUE); // correlation   
    if (0) plotgather_pipe(stemp3,nq,nt,"stemp3");    
    xtimesy(w,Wm,stemp3,nx);
    alphaden=dot(nx,w,s);

    if (alphaden < 0.) err("alphaden=%e\n",alphaden);
    if (alphaden < 1e-10 ){ 
      fprintf(stderr,"alphanum=%e,alphaden=%e,itercg=%d\n",
	      alphanum,alphaden,k);
      //break;
    }
  ...
  // Calculate Jd
  conv_corr(x,stemp,nt,nq,wavelet,nw,FALSE);       // convolution    
  radonf(stemp,r,L,nt,nh,nq,dt,m2,d2,nfft,fmax,0); // Forward
  for (i=0;i<ny;i++) r[i]=Wd[i]*(b[i]-r[i]); 
...
void conv_corr(float *din, float *dout, int nt, int nh, float *wavelet, int nw, int adj)
{
 /* Convolution and correlation: from Claerbout, 1992, Processing vs inversion 
    adj == 0 means convolution
    adj == 1 means correlation  */
