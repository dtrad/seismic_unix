		    ...
		    int nq, int nh, int iter_end, int itercg, int method)
{
  ...
  else if (method==2) vel=1; // q is velocities 
  ...
  c=ealloc1float(nt*nq);

  memset( (void *) m[0], (int) '\0', nq * nt *FSIZE);
  for (iq=0;iq<nt*nq;iq++) c[iq]=1;
  
  na=radonop(nh,h,nt,t,nq,q,A,icol,irow,vel);
  fprintf(stderr,"na=%d\n",na);
  if (!method){
    matmul0(1,na,A,nq*nt,m[0],nh*nt,d[0],irow,icol);  
    matmul0(0,na,A,nq*nt,m[0],nh*nt,d[0],irow,icol);  
  }
  else  PD_lp_bm(nt*nq,nt*nh,na,c,A,m[0],d[0],irow,icol,iter_ext,delta,gamma,PrI,DI,DG,
		 iter_end,itercg,nbig);
