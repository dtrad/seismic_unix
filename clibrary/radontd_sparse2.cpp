  ...
  */
{
  ...
  // Adjoint   
  if (nw) radon3sw(m[0],d[0],index,1,nt,nh,nq,nsparse,wavelet,nw);
  else radon3s(m[0],d[0],index,1,nt,nh,nq,nsparse);
 ...
 
{
  ...
  /* This CG is intended to work wit a very good initial model given bu sinc, and then to 
     use only detail update to improve the result
  */
  ...
  if ((rho=alloc1float(inv.itercg+1))==NULL)
    err("cannot allocate memory for rho\n");  
  if ((eta=alloc1float(inv.itercg+1))==NULL)
