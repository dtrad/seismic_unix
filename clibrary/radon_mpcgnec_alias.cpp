...
void radon_mpcgnec_alias(float *pos, int nh, float **d,float *t,int nt, float dt,float **m, float *q, int nq,float dq,float eps1, float eps2, float eps, float fmax,float *Wd, int itercg, int iter_end, int norm, float step, int testadj, float quantil, int rtmethod)
{
   ...
   for (freq=freq0;freq<maxfreq;freq++){
       ...
       while (iter<iter_end){
         iter++;
	 if ((iter==1)&&(freq==freq0)){ 
	   for (iq=0;iq<nq;iq++) Qp[iq]=1;//eps1; 
	   itermin=25;
	 }
	 else if (freq==freq0){
	   for (i=0;i<nq;i++) mtemp[i]=abs(m2[freq][i]);
	   qmodel=quest(quantil,nq,mtemp);  
	 ...
	 // Use previous model to compute Qp
	   ...
	   iter=iter_end;
	   for (i=0;i<nq;i++) mtemp[i]=abs(m2[freq-1][i]);
	   qmodel=quest(quantil,nq,mtemp);  
	   for (iq=0;iq<nq;iq++) Qp[iq]=MAX(MIN(abs(m2[freq-1][iq])/qmodel,1e2),1e-1);
	   itermin=25;
	 }	 
	 mpcgne(L,nh,nq,m2[freq],d2[freq],Qp,eps,step,itercg,1,eps1,eps2,itermin);
	 fprintf(stderr,"freq=%d=====>power=%f\n",freq-1,qmodel);
	 //	 fprintf(stderr,"AFTERpp**************=%d\n",++pp);  
       }
       /////////////////////////////////////////////////
       if ((wa<1)&&(wa>0)) for (iq=0;iq<nq;iq++)  m2[freq][iq]*=wa;
       //if ((freq>13)&&(freq<17)) m2[freq][iq]*=0.01;
   }
