...

{
  ...
  if (mute1){
    ...
    if (1) plotgather_pipe(modeltemp,nq,nt,"mute1");
    /* The original data is replaced by the predicted data */ 
    hrrti(data,h,dt,modeltemp,q,fmax,nt,nh,nq,rtmethod,depth);
    if (1) plotgather_pipe(data,nh,nt,"mute1");
  }
  

  if (mute2){
    side=1;
    fprintf(stderr,"Muting primaries before q=%f \n",parmute2);
    ...
    if (1){
      ...
      system("suxwigb key=f2 < mutedmodel.su title=mutedmodel perc=99 xbox=600 & \n");
    } 
  
    /* the filtered RT space is used to predict the multiples */
