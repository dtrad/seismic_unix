 ...
 
{
  ...
  float *dp; //temporal array to plot predicted data

  float dt=0.004; // Change this if needed for a given argument 
  int nx=nt*np;
  int ny=nt*nh;  
  ...
  xplotgather(M,np,nt,dt,"Wm.su",0,"perc=100 legend=1");
  ...
  while((rhold>inv.eps)&&(k<inv.itercg)){
    ...
    //  Plots for debugging
    if (PLOT){ 
      xplotgather(x,np,nt,dt,"xiter.su",k,"perc=97 xbox=0 &");
     
      xplotgather(r2,nh,nt,dt,"riter.su",k,"bclip=1.20259 wclip=-1.175 xbox=600 legend=1&");
    
      (*oper) (x,dp,index,0,nt,nh,np,nsparse);
      xplotgather(dp,nh,nt,dt,"dp.su",k,"bclip=1.20259 wclip=-1.175 xbox=600");
    }
