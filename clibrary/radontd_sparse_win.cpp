...
#define NWIN 10 // Number of model space windows
  ...
  */
{
  ...
  /***************************************************************************/
  // Windows: define nwin windows, nt for the model is then nt/5;
  ...
  for (iwin=0;iwin<nwin;iwin++){
    ...
    TRACE;
    it0=iwin*ntwin;
    fprintf(stderr,"it0=%d,iwin=%d,ntwin=%d\n",it0,iwin,ntwin);

    for (iq=0;iq<nq;iq++) for (it=0;it<ntwin;it++) mw[iq][it]=m[iq][it0+it];
    TRACE;   
    if (!LI) build_index_slowness(t,h,q,vgrid,nt,nh,nq,index,it0,ntwin);
    ...
    else if (inv.restart==1){
      TRACE;
      radon3s(mw[0],dtemp[0],index,1,nt,nh,nq,nsparse);
    }
    ...
    for (j=1;j<=inv.iter_end;j++){
      ...
      modelweight(mw[0],nq*ntwin,inv.norm,inv.eps1,Wm[0]);
	...
	wpcgnr(radon3sw,nt,nh,nq,nsparse,m[0],d[0],Wd[0],Wm[0],index,inv,wavelet,nw);
      ...
      else wpcgnr(radon3s,nt,nh,nq,nsparse,mw[0],d[0],Wd[0],Wm[0],index,it0,ntwin,inv);
      if (j==inv.iter_end) inv.itercg/=3;
    }
    if (0){ 
      save_gather(m,nq,nt,dt,"mwin");
      system("suxwigb < mwin  title=\"plot mwin\" &");
    }
    TRACE;
    fprintf(stderr,"it0=%d,ntwin=%d\n",it0,ntwin);

    for (iq=0;iq<nq;iq++) for (it=0;it< ntwin; it++) m[iq][it+it0]=mw[iq][it];
  }
  TRACE;
  if (1){ 
    save_gather(m,nq,nt,dt,"mtotal");
    system("suxwigb < mtotal  title=\"plot mtotal\" &");
  }
  TRACE;
  radonhyp_sinc(m[0],t,h,q,d[0],vgrid,0,nt,nh,nq,0);
...
void build_index_slowness(float *t, float *h, float *q, float **vel, int nt, int nh, int nq,unsigned int **index, int it0, int ntwin)
{
  register int it; // Time index for the model space
  int ih,iq;
  float time,hxh,pxhxh;
  int iqxnt,ihxnt;
  int itime;
  int nsparse=ntwin*nq*nh;
  float dt=t[1]-t[0];
  int itd; // This is the time index for the data space
  


  /* The sparse matrix has times only from it0 to nt, i.e., nt0 elements,
     however the data and model vectors have times from it to nt, hence
  ...
  */
  ...
  for (ih=0;ih<nh;ih++){
	...
	itd=it+it0;
	if (vel[iq][itd]>0){
	  pxhxh=hxh*vel[iq][itd];
	  time=sqrt(t[itd]*t[itd]+pxhxh);
	  itime=(int) (time/dt+0.5);
	  if (itime<nt){
	    index[0][ih*nq*ntwin+iq*ntwin+it]=ihxnt+itime;
	    index[1][ih*nq*ntwin+iq*ntwin+it]=iqxnt+it;
	  }
	  else{
	    index[0][ih*nq*ntwin+iq*ntwin+it]=0;
	    index[1][ih*nq*ntwin+iq*ntwin+it]=0;
	  }
	}
      }            
    }
...
void build_index_slowness_li(float *t, float *h, float *q, float **vel, int nt, int nh, int nq,unsigned int **index, int it0)
{
  ...
  for (ih=0;ih<nh;ih++){
	...
	if (vel[iq][it]>0){
	  ...
	  if (itime<(nt*10)){
	    index[0][ih*nq*ntwin+iq*ntwin+(it-it0)]=10*ihxnt+itime;
	    index[1][ih*nq*ntwin+iq*ntwin+(it-it0)]=iqxnt+it;
	  }
	  else{
	    index[0][ih*nq*ntwin+iq*ntwin+(it-it0)]=0;
	    index[1][ih*nq*ntwin+iq*ntwin+(it-it0)]=0;
	  }
	}
      }            
    }
