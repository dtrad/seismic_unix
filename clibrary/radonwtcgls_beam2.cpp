...
void radon_param_beam_half(float fmax1, float fmax2, float *h, int nh, float *q, 
...
void radonwtcgls_beam2(float *h, int nh, float **data, float *t, int nt, float dt,float **model, float *q, int nq, float dq, float eps1, float eps2, float eps, float *Wd, int itercg, int iter_end,int norm,float step, int testadj, float quantil, int nq1, int nq2, int rtmethod1, int rtmethod2, float depth1, float depth2, float qmin1, float qmin2, float factor1, float factor2, float fmax1, float fmax2)
{
  ...
  for (iq=0;iq<nq1;iq++) Wm2[iq]=0.1;
  for (iq=0;iq<nq2;iq++) Wm2[nq1+iq]=1;
  

  ////////////////////////////////////////////////////

  radon_param_beam_half(fmax1,fmax2,h,nh,q, nq, qmin1,qmin2,q1,q2,ph1,ph2,nq1,nq2,depth1,depth2,rtmethod1, rtmethod2,factor1,factor2,&dq1,&dq2);
  ...
  for (freq=maxfreq1;freq<maxfreq2;freq++){
    ...
    if (rtmethod2==3) matrix(L2,ph2,q2,nh,nq2,w,dq2,rtmethod2);
     ...
     for (iter=1;iter<=iter_end;iter++){
       ...
       if (iter>1) for (iq=0;iq<nq2;iq++) mtemp[iq]=abs(m2[freq][iq]); 
     ...
     for (iq=0;iq<nq1;iq++) m2[freq][iq]=0; 
     for (iq=nq1;iq<nq2;iq++) m2[freq][iq]=m2small[iq];            
     //////////////////////////////////////////////////////////////
