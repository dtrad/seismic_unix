  ...
  */
{
  ...
  int testadj=1;
  float dotwav;
  int LI=0;  // Linear interpolation in the last iteration 
  extern float *rest;
  //////////////////////////////////////////////////////////////////////////
  ...
  void (*radon3sw_li) (float *m, float *d, unsigned int **index, int adj, int nt, 
		   int nh, int nq, int nsparse, float *wavelet, int nw);

  void (*radon3s_li) (float *m, float *d, unsigned int **index, int adj, int nt, 
		   int nh, int nq, int nsparse);
  ...
  radon3sw=radonhyp;
  radon3s=radonhyp;
  radon3sw_li=radonhyp_li;
  radon3s_li=radonhyp_li;
  convop=contruc_2;
  //convop=contran;
  ...
  TRACE;
  rest=ealloc1float(nt);
  TRACE;
  // Assign the elements of index

  build_index_slowness_li(t,h,q,vgrid,nt,nh,nq,index,it0);
  ...
  /****************The first iterations are simple nearest nighbor, the last one is ***
  ...
  if (LI){
    /**** Last iteration with linear interpolation ****/
    modelweight(m[0],nq*nt,inv.norm,inv.eps1,Wm[0]);
    if (nw){ 
      fprintf(stderr,"iteration with wavelet convolution\n");
      wpcgnr(radon3sw_li,nt,nh,nq,nsparse,m[0],d[0],Wd[0],Wm[0],index,inv,wavelet,nw);
    }
    else wpcgnr(radon3s_li,nt,nh,nq,nsparse,m[0],d[0],Wd[0],Wm[0],index,inv);
  }
  else{
    /**** Last iteration with nearest neighbor  ****/
    modelweight(m[0],nq*nt,inv.norm,inv.eps1,Wm[0]);
    if (nw){ 
      fprintf(stderr,"iteration with wavelet convolution\n");
      wpcgnr(radon3sw,nt,nh,nq,nsparse,m[0],d[0],Wd[0],Wm[0],index,inv,wavelet,nw);
  ...
  if (nw) radonhyp_li(m[0],d[0],index,0,nt,nh,nq,nsparse,wavelet,nw);
  else radonhyp_li(m[0],d[0],index,0,nt,nh,nq,nsparse);

  /***********************************************************************************/

  // Let us kill the monster
  free2((void **) index);
  free1float(rest);rest=0;  
  free2float(vgrid);  
  free2float(Wm);
  free2float(Wd);  
...
void build_index_slowness_li(float *t, float *h, float *q, float **vel, int nt, int nh, int nq,unsigned int **index, int it0)
{
  ...
  for (ih=0;ih<nh;ih++){
	...
	if (vel[iq][it]>0){
	  ...
	  //rest[ih*nq*(nt-it0)+iq*(nt-it0)+it00]=time-itime;
