  ...
  */
{
  ...
  for (iwin=0;iwin<nwin;iwin++){  
    ...
    if ((plot==1)||(plot==3)){
      ...
      system("suxwigb < dw  perc=99  title=\"CSP\" xbox=600 &" );
    }
    ...
    if ((plot==1)||(plot==3)){
      ...
      system("suxwigb < dw perc=99 key=offset  title=\"CSP After RT\" ");
    }
...
void radontd_sparse_win(float *t, float *q, float *h, float **m,float **d, int nt, int nh, int nq, float dt, float **vgrid, inv_par inv, int dataprec, int nw, float fpeak, int typewav, int LI, float parmute, int mute, int itm, int plot)
  ...
  /*
    RADONTD_SPARSE_WIN
    RADON TRANSFORM INSIDE A WINDOW IN THE TIME DOMAIN WITH DATA WEIGHTS
  ...
  */
{
  ...
  if ((index=(unsigned int ***) alloc2(nsparse,3,size))==NULL) 
    err("Cannot allocate index\n");

  // Assign the elements of index
  if (!LI) build_index_slowness(t,h,q,d,vgrid,nt,nh,nq,index);
  else build_index_slowness_li(t,h,q,d,vgrid,nt,nh,nq,index);
  ...
  for (ih=0;ih<nh;ih++){
    ...
    for (iq=0;iq<nq;iq++){    
	  ...
	  if (itime<nt){
	    ...
	    index[2][ih*nq*nt+iq*nt+it]=d[ih*nt+it];
	  }
	  else{
	    index[0][ih*nq*nt+iq*nt+it]=0;
	    index[1][ih*nq*nt+iq*nt+it]=0;
	    index[2][ih*nq*nt+iq*nt+it]=0;
	  }
      }            
    }
  }
