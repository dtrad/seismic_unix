...
#define PLOT 0 // =1 plot every gather and stop
...
void radontd_sparse(float *t, float *q, float *h, float **m,float **d, int nt, int nh, int nq, float dt, float *vel, float dperv, float pervmin, float t0,  inv_par inv, int centralq, int dataprec, int nw, float fpeak, int typewav, int LI, int nreg, float parmute, int mute)
  ...
  */
{
  ...
  fprintf(stderr,"t0=%f,it0=%d,dt=%f,t[%d]=%f\n",t0,it0,dt,it0_total,t[it0_total]);
  ...
  for (iwin=0;iwin<nwin;iwin++){  
    ...
    radontd_sparse_win(&t[it0],q,h,mw,dw,ntwin,nh,nq,dt,vgridw,inv,dataprec,nw,fpeak,typewav,LI,parmute,mute);   

    if (PLOT){
      save_gather(mw,nq,ntwin,dt,"mw");
      system("suxwigb < mw perc=97 key=f2 title=\"plot mw\" ");
    }
    if (PLOT){
      save_gather(dw,nh,ntwin,dt,"dw");
      system("suxwigb < dw perc=97 key=offset  title=\"plot dw\" ");
    }
  ...
  */
{
  ...
  if (mute){  
    if (nw) rest_multiples(radon3sw,m,d,index,0,nt,nh,nq,nsparse,wavelet,nw,parmute,q);
    else rest_multiples(radon3s,m,d,index,0,nt,nh,nq,nsparse,parmute,q);
  }
  else{
