	     ...
	     float eps1, float eps2, float fmax)
{
   ...
   float w,wa,*dh,df,eps=1e-4,testcholres,*Jtot,Jmod,Jdata,powd,*gx;
   ...
   for (freq=1;freq<maxfreq;freq++){
	 ...
	 if (iter==1){
           ...
           //fprintf(stderr,"iter==%d , eps2=%e LS\n",iter,eps2); 
 	   for (i=0;i<nq;i++) ll[i][i].r*=(1+eps1/100);          
         }
         if (iter!=1){
	   eps2=modgrad(maux2,nq,norm,1.,Qp);
	 ...
	 for (i=0;i<nh;i++) 
	   ll2[i][i].r+=(eps1/100*ll2[i][i].r+1e-10);
