...
void wtcgls0(float *pos, float **data, float dt,float **model, float *q, float dq,float eps1, float eps2, float eps, float fmax, float *Wd)
{
  ...
  eps1/=100;
   ...
   for (freq=1;freq<maxfreq;freq++){
	 ...
	 if ((iter!=1)){
       ...
       wtcgls(d,L,LH,u,Wm,Wd,nh,nq,qcg,q1cg,scg,x1cg,zcg,z1cg,rcg,Azcg,
	      eta,rho,gcv,eps);
     }
     for (iq=0;iq<nq;iq++) uaux[iq]=abs(u[iq]);            
     /////////////////////////////////////////////////
