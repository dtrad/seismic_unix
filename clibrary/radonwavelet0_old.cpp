...
void radonwavelet0(float **data, float *t, int nt, float *h, int nh, float **model, float *q, int nq, int wavn, float quantil, int scalefilt)
{
  ...
  if (1){
    ...
    for (iw=fw;iw<nw;iw++){
      scale=iw;
      radonwindow2(data,t,nt,h,nh,scale,itd,datatemp);
      sprintf(buf2,"scale%d",scale);
      save_gather(datatemp,nh,nt,dt*nt/itd[scale],buf2);
...
void radonwindow2(float **data, float *t, int nt, float *h, int nh, int scale, int *itd, float **datatemp)
{
  float **dataw;
  int ntw=itd[scale];
