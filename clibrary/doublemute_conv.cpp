...

{
  ...
  if (mute2){
    ...
    if (1){
      ...
      system("suxwigb key=offset < multiples.su title=nmo_multiples perc=99 &");
    } 
    if ((mute1)||(mute2)){
...

{
  ...
  modeltemp=ealloc2float(nt,nq);
  modeltemp2=ealloc2float(nt,nq);
  datarec=ealloc2float(nt,nh);
  datanmo=ealloc2float(nt,nh);
  ...
  if (mute2){
    ...
    conv_corr(modeltemp[0],modeltemp2[0],nt,nq,wavelet,nw,FALSE); 
    hrrti(datarec,h,dt,modeltemp2,q,fmax,nt,nh,nq,rtmethod,depth);
    if (plot){
      dtemp=ealloc1float(nt);
      for (ih=0;ih<nh;ih++){
	for (it=0;it<nt;it++) dtemp[it]=datarec[ih][it];    
	nmo(datanmo[ih],dtemp,t,nmofactor*h[ih],vel,1,nt,dt,smute);  
      }
      free1float(dtemp);    
      save_gather(datanmo,nh,h,nt,dt,"multiples.su");      
      system("suxwigb key=offset < multiples.su title=nmo_multiples perc=99 &");
    } 
    if ((mute1)||(mute2)){
      fprintf(stderr,"substracting multiples \n");
