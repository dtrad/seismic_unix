...
void stoltz_wtcgls(float **data, float **model, float **Wd, float *h, int nh,  float *t, int nt,
		   float *x, int nx, float vel, int nv, float dv, inv_par inv, complex **F, complex **F2, float *velarray, int nv)
{
  ...
  void (*oper) (float **datain, float **dataout, int nt, int nh, int nx, float *t, 
		float *h, float *x, float vel, complex **F, complex **F2, float *velarray, int nv,
		int adj);

  if (1) adjtestconvgather(nx,nt,velarray,nv);
  fprintf(stderr,"nt=%d\n",nt);  
  ...
  for (j=1;j<=inv.iter_end;j++){
    ...
    J[j-1]=wpcgnr_mig(oper,model,data,nt,nh,nx,t,h,x,vel,Wm,Wd,inv,F,F2,wavelet,nw);
