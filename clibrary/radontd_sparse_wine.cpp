...
#include "radontd_sparse_win.h"
#define NWIN 1 // Number of model space windows


void radonhyp_sinc(float *m,float *t, float *h, float *q, float *d, float **vel,int adj,int nt, int nh, int nq);

void radontd_sparse_win(float *t, float *q, float *h, float **m,float **d, int nt, int nh, int nq, float dt, float **vgrid, inv_par inv, int dataprec, int nw, float fpeak, int typewav, int LI);
  ...
  */
{
  ...
  for (iwin=0;iwin<nwin;iwin++){
    ...
    radontd_sparse_win(&t[it0],q,h,mw,dw,ntwin,nh,nq,dt,vgridw,inv,dataprec,nw,fpeak,typewav,LI);   

    if (0){
      save_gather(mw,nq,ntwin,dt,"mw");
