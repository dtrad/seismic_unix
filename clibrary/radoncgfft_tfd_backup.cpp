...
void radoncgfft_tfd(float *pos, int nh, float **data, float *t, int nt, float dt,
		    ...
		    int testadj, float quantil, int rtmethod,  float depth, inv_par inv)
{
  ...
  for (j=1;j<=inv.iter_end;j++){
    ...
    J[j-1]=cgfft(data[0],L,model[0],d2,RC,m2,nt,nh,nq,nf2,dt,nfft,fmax,Wm[0],Wd[0],inv);
  }
 ...
 
{
  ...
  float *r,*g,*s,*stemp,*stemp2,*w,*rho,*eta,rhold, *rc, *xc;
  complex **m2aux;
  int nx=nt*nq;
  int ny=nt*nh;  
  ...
  for(i=0;i<nx;i++) g[i]=g[i]*Wm[i];


  normb=dot(nx,g,g);
  rho[0]=normb;      
  for(i=0;i<nx;i++) s[i]=g[i];
  rhold=normb*2; // Arbitrary first choice of rhold > rho
  k=0;
  while(k<inv.itercg){
    k++;
    // Calculate alpha num
    ...
    Atimesx_by_fft_loop(stemp2,RC,stemp,m2,m2aux,nt,nq,dt,nfft,fmax,nf2);
    if (0) plotgather_pipe(stemp2,nq,nt,"stemp2");    
    xtimesy(w,Wm,stemp2,nx);
    alphaden=dot(nx,w,s);
