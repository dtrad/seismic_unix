...
//--      g1=d-LH*L*dsol
	 ...
	 eps2=modgrad(u,nq,norm,power,Qp);
         //displayA(Qp,nq);
	 //for (i=0;i<nq;i++) if (Qp[i].r>(1./eps))  Qp[i].r/=eps2;  // test
...
//- NQ loop
	    ...
	    ///// Compute Beta
	      ...
	      //fprintf(stderr,"beta=%g k=%d\n",beta,k);
            ...
            Atimesx(utemp,LH,g2,nq,nh);
            xtimesy(utemp2,Qp,utemp,nq); 
            Atimesx(gtemp,L,utemp2,nh,nq);
            for (i=0;i<nh;i++) gtemp[i]*=(1-eps1/100.);            
