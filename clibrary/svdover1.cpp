	     ...
	     float eps1, float eps2, float eps, float fmax)
{
   ...
   x=vector(1,nn);
   /*
   if ((V=alloc2float(nn,nn))==NULL)
     err("cannot allocate memory for V\n");
   if ((W=alloc1float(nn))==NULL)
     err("cannot allocate memory for W\n");
   if ((RHS=alloc1float(nn))==NULL)
     err("cannot allocate memory for RHS\n");
   if ((x=alloc1float(nn))==NULL)
   ...
   for (freq=2;freq<maxfreq;freq++){
	 ...
	 for (j=0;j<nq;j++)
	   ...
	   RHS[i+1]=maux[i].r;
         ...
         svdcmp(U,nn,nn,W,V);
         wmax=0.0;
         for (i=0;i<nn;i++) if (abs(W[i]) > wmax) wmax=W[i];
         wmin=1e-2*wmax;
         for (i=0;i<nn;i++)
         fprintf(stderr,"fre=%d,W[%d]=%e\n",freq,i,W[i]);       
         for (i=0;i<nn;i++) if (W[i]<wmin) W[i]=0.0;        
         svbksb(U,W,V,nn,nn,RHS,x);
         for (i=0;i<nq;i++){
	   maux2[i].r=x[i+1];
	   maux2[i].i=x[nq+i+1];           
         } 
