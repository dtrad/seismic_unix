...
char *sdoc[] = {
  ...
  " keepdata=1      =1 to output only new traces, =0 output all         "
...
int main(int argc, char **argv)
{
  ...
  if (!getparfloat("parmute",&parmute)) parmute=1e-7;
  ...
  if (!getparint("keepdata",&keepdata)) keepdata=0;
  ...
  if (tr.cdp>=cdpmin) traux=tr; // Keep traux for use later
  else{ 
    do {
      if (verbose) fprintf(stderr,"tr.cdp=%d\n",tr.cdp);
    } while ((tr.cdp < cdpmin) && (ngettr=gettr(&tr)));
  ...
  for (ix=0;ix<nx;ix++){
    ...
    } while ((tr.cdp <= x[ix]) && (ngettr=gettr(&tr)));
    traux=tr;
    erewind(tracefp);
    erewind(headerfp);  
    fprintf(stderr,"ntr=%d\n",ntr);
    ...
    nhcmp[ix]=ih;
    // when the file is empty and gettr returns 0 read bytes
    // nhcmp[ix] has a value one less than number of cmp traces
    // To fixed it I increased here, (works fine)
    // I don't know a  better solution for now. 
    if (!ngettr){
      nhcmp[ix]++;
      he[nhcmp[ix]-1]=tr.offset;
    }
    ///////////////////////////////////////////////////
    ...
    // If mute==1 means that the Radons space has been altered by muting and
    // the original traces must be replaced by the new ones.
    if ((mute==0)&&(keepdata)) sethflag2(hflag,nhe,he,nhe,h2,nh2,under,over);
    ...
    if (mute==1)  // Number of computed traces = original 
