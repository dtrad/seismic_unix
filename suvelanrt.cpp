...
int main(int argc, char **argv)
{
  ...
  if (!getparfloat("Vmax", &Vmax))  Vmax = 10000;
  ...
  do {
    ...
    h[j]=(float) tr.offset;
    for (i=0;i<nt;i++){
      d[i+nt*j]=(float) tr.data[i];
    }
    j++;
  } while (gettr(&tr));
  fprintf(stderr,"Before velanrt nq=%d, nt=%d, nh=%d\n",nq,nt,nh);  
  nh=j;
  ny=nt*nh;
  nx=nt*nq;
  velanrt(t,q,h,m,d,eps,Vmin,Vmax);
  fprintf(stderr,"After velanrt nq=%d, nt=%d, nh=%d\n",nq,nt,nh);
